...
from chat.gemini import make_ai_response, stream_ai_response
...
def sendMessageStream(request):
    logger.info("into sendMessageStream")
    if request.method == "POST":
        userInputText = request.POST.get("userInputText", "").strip()
        all_conv_ids = []
        i = 0
        ...
        if not userInputText:
            return JsonResponse({"message": "Failed", "error": "Empty input"}, status=400)

        stream_response = stream_ai_response(userInputText, all_conv_ids)
